...
class BoomMap(object):
    ...
    def show_playerboom(self, row, col):
        for i,boom in enumerate(self.boom_pos()):
            self._board[row + boom[0]][col + boom[1]] = str(self.boom_posnum(row + boom[0], col + boom[1]))
        self.show_board()
...
def main():
    ...
    input_pos = kabi.boom_posnum(guess_row, guess_col)
    pos_type = kabi._board[guess_row][guess_col]
    print(pos_type)
    count = kabi.show_playerboom(guess_row,guess_col)
